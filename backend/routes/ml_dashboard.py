...
def status():
    ...
    return (
        jsonify({"status": "online", "message": "ML Dashboard rodando corretamente"}),
        200,
    )
...
def get_dados():
    ...
    try:
        ...
        pedidos = (
            Pedido.query.filter(
                (Pedido.valor_estimado == None) | (Pedido.valor_estimado == 0)
            )
        ...
        if len(pedidos) < min_each:
            pedidos_extra = (
                ...
                .limit(max(limit, min_each))
                .all()
            )
            pedidos = pedidos_extra[:limit]

        chamados_payload = [
            c.to_dict() if hasattr(c, "to_dict") else vars(c) for c in chamados
        ]
        ...
        for chamado_data in dados.get("chamados", []):
            try:
                ...
                resultados["erros"].append(
                    f"Chamado {chamado_data.get('id')}: {str(e)}"
                )
