...
def get_dashboard():
    try:
        ...
        receita_total = db.session.query(func.sum(Pedido.valor_estimado)).select_from(Pedido).scalar() or 0
...
def get_ml_exemplos():
    ...
    for modelo_nome, cfg in modelos.items():
        exemplos_por_modelo[modelo_nome] = [
            {
                "exemplo_idx": i,
...
def get_relatorio_ml():
    try:
        ...
        prioridades = db.session.query(
            ChamadoSuporte.prioridade, func.count(ChamadoSuporte.id)
        ).group_by(ChamadoSuporte.prioridade).all()

        tipos_servico = db.session.query(
            Pedido.tipo_servico, func.count(Pedido.id), func.avg(Pedido.valor_estimado)
        ).group_by(Pedido.tipo_servico).all()

        orcamentos_trend = db.session.query(
            Orcamento.tipo, func.avg(Orcamento.valor_estimado), func.count(Orcamento.id)
        ).group_by(Orcamento.tipo).all()

        return jsonify({
            "classificador_suporte": {
                "categorias": [{"categoria": cat, "count": count} for cat, count in categorias],
                "prioridades": [{"prioridade": pri, "count": count} for pri, count in prioridades],
            },
            "recomendador_servicos": [{
                "tipo": tipo,
                "count": count,
...
def promover_admin(usuario_id):
    try:
        ...
        usuario.role = 'admin'
        db.session.commit()
        return jsonify({"message": "Usuário promovido a admin", "usuario": usuario.to_dict()}), 200
    except Exception as e:
        db.session.rollback()
        return jsonify({"error": str(e)}), 500
...
def get_mensagens_suporte():
    try:
        subq = (
            ...
            ).group_by(ChamadoSuporte.usuario_id).subquery()
        )

        ultimos = (
            ChamadoSuporte.query.join(
                subq,
                (ChamadoSuporte.usuario_id == subq.c.usuario_id) & (ChamadoSuporte.data == subq.c.ultima_data)
            ).order_by(ChamadoSuporte.usuario_id.asc(), ChamadoSuporte.id.desc()).all()
        )
...
def update_pedido_status(pedido_id):
    ...
    try:
        ...
        db.session.commit()
        return jsonify({"message": "Status atualizado com sucesso", "pedido": {"id": pedido.id, "status": pedido.status}}), 200
    except Exception as e:
        db.session.rollback()
        return jsonify({"error": str(e)}), 500
